...
async function init() {
  ...
  for (let num = 0; num < 3; num++) {
    ...
    const url = "celeb/" + results[0].label + ".jpg";
    ...
    //console.log("filtered", cur_ffd);
  }

  const redo = new Array();
  for (var i = 0; i < labeledFaceDescriptors.length; i++) {
    const cur = labeledFaceDescriptors[i];
    console.log(cur);
    redo.push([cur.label, Array.from(cur.descriptors[0].values())]);
    //faceapi.LabeledFaceDescriptors
  }
  document.getElementById("data").innerHTML = JSON.stringify(redo);
}
